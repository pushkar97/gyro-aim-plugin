// Vendored & trimmed from GoldHEN_Plugins_Repository/plugin_src/gamepad_helper/source/config.c
// Original attribution (per upstream source comment): https://github.com/Teklad/tconfig
// via https://github.com/gimli2/tconfig
// Removed the ScePadButton/vibration-intensity helpers (pad-specific, not
// needed by this plugin's config schema) and added ini_table_get_entry_as_float.
#include <limits.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static ini_entry_s* _ini_entry_get(ini_table_s* table, const char* section_name, const char* key) {
    ...
    return _ini_entry_find(section, key);
}
...
bool ini_table_get_entry_as_bool(ini_table_s* table, const char* section_name, const char* key,
    ...
    const char* val = ini_table_get_entry(table, section_name, key);
    if (val == NULL) return false;
    *value = (strcasecmp(val, "on") == 0 || strcasecmp(val, "true") == 0 ||
               strcasecmp(val, "1") == 0);
    return true;
}
